...
public class DebugGuiTickHandler implements Serializable, TreeExpansionListener, MouseListener {
	...
	public void startWatchingOf(Object object, EntityPlayer player) {
		...
		LPChannel.sendPacketToPlayer(PacketHandler.getPacket(DebugPanelOpen.class).setName(object.getClass().getSimpleName()), player);
		...
		LPChannel.sendPacketToPlayer(PacketHandler.getPacket(DebugTypePacket.class).setToTransmit(setting.var), player);
	}
	...
	public void expandGuiAt(Integer[] tree, EntityPlayer player) {
		...
		LPChannel.sendPacketToPlayer(PacketHandler.getPacket(DebugTypePacket.class).setToTransmit(pos).setPos(tree), player);
	}
	...
	private VarType handleUpdate(VarType type, EntityPlayer player, LinkedList<Integer> path, Object newObject, ParentVarType parent) {
		...
		if (type instanceof BasicVarType) {
			...
			if (isModified) {
				...
				LPChannel
						.sendPacketToPlayer(PacketHandler.getPacket(DebugInfoUpdate.class).setPath(path.toArray(new Integer[] {})).setInformation(type), player);
			}
			return bType;
		} else if (type instanceof NullVarType) {
			if (newObject != null) {
				type = resolveType(newObject, type, type.name, true, parent);
				LPChannel.sendPacketToPlayer(PacketHandler.getPacket(DebugTypePacket.class).setToTransmit(type).setPos(path.toArray(new Integer[] {})), player);
			}
			return type;
		} else if (type instanceof ArrayVarType) {
			ArrayVarType aType = (ArrayVarType) type;
	...
	public void handleTargetRequest() {
		...
		if (box == null) {
			LPChannel.sendPacketToServer(PacketHandler.getPacket(DebugTargetResponse.class).setMode(TargetMode.None));
		} else if (box.typeOfHit == MovingObjectType.BLOCK) {
			LPChannel.sendPacketToServer(PacketHandler.getPacket(DebugTargetResponse.class).setMode(TargetMode.Block)
			...
			LPChannel.sendPacketToServer(
					PacketHandler.getPacket(DebugTargetResponse.class).setMode(TargetMode.Entity).setAdditions(new Object[] { box.entityHit.getEntityId() }));
		}
	}
	...
	public void targetResponse(TargetMode mode, final EntityPlayer player, Object[] additions) {
		if (mode == TargetMode.None) {
			...
			if (tile == null) {
				...
				LPChatListener.addTask(new Callable<Boolean>() {
					...
					public Boolean call() throws Exception {
						...
						LPChannel.sendPacketToPlayer(PacketHandler.getPacket(OpenChatGui.class), player);
						return true;
					}
				}, player);
			...
			if (entitiy == null) {
				...
				player.addChatComponentMessage(new ChatComponentText(ChatColor.AQUA + "Start debuging of Entity: " + ChatColor.BLUE + ChatColor.UNDERLINE + entitiy.getClass().getSimpleName() + ChatColor.AQUA + "? " + ChatColor.RESET + "<" + ChatColor.GREEN + "yes" + ChatColor.RESET + "/" + ChatColor.RED + "no"
						+ ChatColor.RESET + ">"));
				LPChannel.sendPacketToPlayer(PacketHandler.getPacket(OpenChatGui.class), player);
			}
		}
	...
	public void treeExpanded(TreeExpansionEvent treeexpansionevent) {
		if (treeexpansionevent.getPath().getLastPathComponent() instanceof ExtendedDefaultMutableTreeNode && ((ExtendedDefaultMutableTreeNode) treeexpansionevent.getPath().getLastPathComponent()).type != null) {
			...
			LPChannel.sendPacketToServer(PacketHandler.getPacket(DebugDataPacket.class).setTree(path.toArray(new Integer[] {})));
		}
	}
			...
			if (tree.getLastSelectedPathComponent() instanceof ExtendedDefaultMutableTreeNode) {
				...
				edit.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						LPChannel.sendPacketToServer(PacketHandler.getPacket(DebugSetVarContent.class).setPath(path.toArray(new Integer[] {})).setContent(s));
					}
				});
				popup.add(edit);
	...
	public static DebugGuiTickHandler instance() {
		if (DebugGuiTickHandler.instance == null) {
			DebugGuiTickHandler.instance = new DebugGuiTickHandler();
		}
		return DebugGuiTickHandler.instance;
	}
}
